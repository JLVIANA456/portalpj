...
export function getClients(tenantId: string = 'default'): Client[] {
  const saved = localStorage.getItem(`portal_pj_clients_${tenantId}`);
...
export function saveClients(tenantId: string = 'default', clients: Client[]) {
  localStorage.setItem(`portal_pj_clients_${tenantId}`, JSON.stringify(clients));
}
